...
exports.face_detect = function () {

	var array_group = function (seq, gfunc) {
			var i, j;
		...
		initCascadeOrigFeatures = function () {
		...
		initParams = function (params, names) {
			...
			params = get_named_arguments(params, names);
			params.canvas = grayscale(params.canvas);
	...
	initCascadeOrigFeatures();

	return {
		detect_objects: function (canvas, interval, min_neighbors) {
			var params = initParams(arguments, ["canvas", "interval", "min_neighbors"]),
				interval = params.interval;

			function pre(interval, params) {
				var canvas = params.canvas;
				var scale = params.scale;
				var next = params.next;
				var scale_upto = params.scale_upto;

				function createCanvas(pyr, i, next) {
					return new Canvas(
				...
				function createRet(pyr, index) {
					return {
						"width": pyr[index].width,
						"height": pyr[index].height,
						"data": pyr[index].getContext("2d").getImageData(0, 0, pyr[index].width, pyr[index].height).data
					};
				}
				...
				pyr[0] = canvas;
				ret[0] = createRet(pyr, 0);
				var i;
				for (i = 1; i <= interval; i++) {
					var index = (i * 4);
					pyr[index] = new Canvas(
						Math.floor(pyr[0].width / Math.pow(scale, i)),
						Math.floor(pyr[0].height / Math.pow(scale, i))
					);
					pyr[index].getContext("2d").drawImage(pyr[0], 0, 0, pyr[0].width, pyr[0].height, 0, 0, pyr[i * 4].width, pyr[i * 4].height);
					ret[index] = createRet(pyr, index);
				}
				for (i = next; i < scale_upto + next * 2; i++) {
					var index = (i * 4);
					pyr[index] = createCanvas(pyr, i, next);
					pyr[index].getContext("2d").drawImage(pyr[i * 4 - next * 4], 0, 0, pyr[i * 4 - next * 4].width, pyr[i * 4 - next * 4].height, 0, 0, pyr[i * 4].width, pyr[i * 4].height);
					ret[index] = createRet(pyr, index);
				}
				for (i = next * 2; i < scale_upto + next * 2; i++) {
					var index = (i * 4 + 1);
					pyr[index] = createCanvas(pyr, i, next);
					pyr[index].getContext("2d").drawImage(pyr[i * 4 - next * 4], 1, 0, pyr[i * 4 - next * 4].width - 1, pyr[i * 4 - next * 4].height, 0, 0, pyr[index].width - 2, pyr[index].height);
					ret[index] = createRet(pyr, index);

					var sndex = (i * 4 + 2);
					pyr[sndex] = createCanvas(pyr, i, next);
					pyr[sndex].getContext("2d").drawImage(pyr[i * 4 - next * 4], 0, 1, pyr[i * 4 - next * 4].width, pyr[i * 4 - next * 4].height - 1, 0, 0, pyr[sndex].width, pyr[sndex].height - 2);
					ret[sndex] = createRet(pyr, sndex);

					var tndex = (i * 4 + 3);
					pyr[tndex] = createCanvas(pyr, i, next);
					pyr[tndex].getContext("2d").drawImage(pyr[i * 4 - next * 4], 1, 1, pyr[i * 4 - next * 4].width - 1, pyr[i * 4 - next * 4].height - 1, 0, 0, pyr[tndex].width - 2, pyr[tndex].height - 2);
					ret[tndex] = createRet(pyr, tndex);
				}
				return ret;
			}
			...
			return post([core(pre(interval, params))]);
		}
	}
};